...
def renderText(
    ...
    textRendered: pygame.Surface = font.render(
        text, True, NAME_SPACE["color"][color]
    )
...
class Button:
    def __init__(
        ...
        *,
        ...
        assert innerColor in NAME_SPACE["color"], (
            f"Invalid background color: {innerColor}"
        )
        assert textColor in NAME_SPACE["color"], (
            f"Invalid text color: {textColor}"
        )
        assert hoverColor in NAME_SPACE["color"], (
    ...
    def render(self, surface: pygame.Surface, mousePos: tuple[int]) -> None:
        ...
        pygame.draw.rect(
            surface,
            NAME_SPACE["color"][
                self.hoverColor if isHovered else self.innerColor
            ],
            self.innerRect,
        )
        # Buttons border
...
class Inventory:
    def __init__(self, *args) -> None:
        ...
        self.inventory: dict[int, Item] = dict.fromkeys(range(FIX_STGS["inventoryCol"] * FIX_STGS["inventoryRow"]))
        ...
        self.innerRects: list[pygame.Rect] = [
            pygame.Rect(
                ...
                + (i % FIX_STGS["inventoryCol"])
                * (innerRectSize + FIX_STGS["GUI"]["slotPadding"]),
                FIX_STGS["GUI"]["outerWindowPadding"]
                + (i // FIX_STGS["inventoryCol"])
                * (innerRectSize + FIX_STGS["GUI"]["slotPadding"]),
                innerRectSize,
        ...
        self.notHoveredInner = pygame.Surface(
            (innerRectSize, innerRectSize), pygame.SRCALPHA
        )
        self.notHoveredInner.fill((
            *NAME_SPACE["color"]["buttonInner"],
            FIX_STGS["GUI"]["slotTransparency"],
        ))

        self.hoveredInner = pygame.Surface(
            (innerRectSize, innerRectSize), pygame.SRCALPHA
        )
        self.hoveredInner.fill((
            *NAME_SPACE["color"]["buttonHover"],
            FIX_STGS["GUI"]["slotTransparency"],
        ))

        self.borderRects: list = [
            pygame.Rect(
                FIX_STGS["GUI"]["outerWindowPadding"]
                - FIX_STGS["GUI"]["slotBorderWidth"]
                + (i % FIX_STGS["inventoryCol"])
                * (innerRectSize + FIX_STGS["GUI"]["slotPadding"]),
                FIX_STGS["GUI"]["outerWindowPadding"]
                - FIX_STGS["GUI"]["slotBorderWidth"]
    ...
    def renderHotbar(
        self, surface: pygame.Surface, hotbarNum: int, mousePos: tuple[int]
    ) -> None:
        # Actual inventory
        for slotNum in range(10):  # The number of slots in a row
            ...
            if item is not None:
                item.renderIcon(
                    ...
                    (
                        int(
                            self.innerRects[slotNum].x
                            + self.innerRects[slotNum].w * 0.5
                            - STGS["guiSize"] * 0.5
                        ),
                        int(
                            self.innerRects[slotNum].y
                            + self.innerRects[slotNum].h * 0.5
                            - STGS["guiSize"] * 0.5
                        ),
                    ),
                )

    def renderFullInventory(
        self, surface: pygame.Surface, hotbarNum: int, mousePos: tuple[int]
    ) -> None:
        # Actual inventory
...
class CursorSlot:
    ...
    def renderItemAtCursor(
        self, surface: pygame.Surface, mousePos: tuple[int]
    ) -> None:
        item = self.slot
        if item is not None:
            item.renderIcon(
