...
def generateMap(
    xLeft: int = 0, xRight: int = 100, yTop: int = 0, yBottom: int = 100
    ...
    for x in range(xLeft, xRight):
        ...
        for y in range(yTop, tilDirtY):
            tilemap[f"{x};{y}"] = {"block": "dirt", "variant": 0}

        # Stone
        for y in range(tilDirtY, yBottom):
            tilemap[f"{x};{y}"] = {"block": "stone", "variant": 0}
    ...
    def drawPatchStroke(
        ...
        if count == 0:
            if block == "stone":
                patchesInStroke: int = int(
                    diff25(myMathFunc1((point[1] - yTop) / (tilDirtY - yTop)))
                )
            elif block == "dirt":
                patchesInStroke: int = int(
                    diff25(
                        myMathFunc2(
                            (point[1] - tilDirtY) / (yBottom - tilDirtY)
                        )
                    )
                )
            patchesInStroke = min(
                max(patchesInStroke, 0), 32
            )  # Capped between 0 and 32

        if patchesInStroke != 0:
            # Go thru the circle and draw it
            for x in range(int(point[0] - radius), int(point[0] + radius) + 1):
                for y in range(
                    int(point[1] - radius), int(point[1] + radius) + 1
                ):
                    ...
                    if (x - point[0]) ** 2 + (y - point[1]) ** 2 <= roundUp(
                        radius**2
                    ):
                        tilemap[f"{x};{y}"] = {"block": block, "variant": 0}
        ...
        if count < patchesInStroke:
            ...
            nextRadius: float = min(
                max(diff25(radius), 1), 5
            ...
            drawPatchStroke(
                ...
                count=count + 1,
                radius=nextRadius,
                patchesInStroke=patchesInStroke,
            )

    # Stone patches in dirt
    numOfPatches: int = int(
        diff25((xRight - xLeft) * (tilDirtY - yTop) * 0.003)
    )
    ...
    numOfPatches: int = int(
        diff25((xRight - xLeft) * (yBottom - tilDirtY) * 0.004)
    )
    ...
    worldOres: list[str] = [
        random.choice(ores[(2 * _) : ((_ + 1) * 2)])
        for _ in range(int(len(ores) / 2))
    ]
